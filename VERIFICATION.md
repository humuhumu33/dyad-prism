...
| Verify | a theorem Lean cannot close by the stated proof, or a declaration whose observed axioms differ from its pinned exact set (`tools/axioms.json`) | build | pending |
...
| Words | a `shell/index.html` whose words drifted from `view()` | build | pending |
| Bytes | a generated function whose bytes differ from the rule on any corpus case (`model/corpus.json`) | build | pending |
