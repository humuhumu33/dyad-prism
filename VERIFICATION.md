...
| Contract | a host answer whose shape is not what Dyad's own contract declares: the Pages build refuses a literal of the wrong shape (`tools/contract_shapes.mjs --check`, run where the renderer's dependencies are installed), and the page parses every answer against the contract it publishes (`window.__contracts`), names the channel in the console and hands an iterable screen an empty array rather than a value it cannot walk | build | armed 2026-09-15 by the bug that found it: `get-version-changes` answered `{files: []}` where the contract declares an array, and a version's changed files card crashed the app with "is not iterable"; six more disagreements were then found and corrected |
