...
`https://humuhumu33.github.io/dyad-prism/ui/` is Hollama (fmaclen/hollama at tag 0.35.4, MIT), built as static files under this subpath and branded with the kit: the warm ground, the accent once per view, Geist, the H. It talks to the OpenAI compatible endpoint this origin's worker answers, so the model runs in the visitor's own browser and no server is anywhere. One link and nothing to set up: the visitor lands in a new session, the connection to this origin is already there, the model starts loading at once with the first download shown, and the picker is filled in. `scripts/build-hollama.sh` reproduces `shell/ui/` from the pinned commit, `scripts/hollama-brand.patch` (names, links, asset paths, the subpath, the static adapter, the landing), `tools/kit-to-hsl.mjs` (the kit's colours, fonts and mark) and `scripts/ui-host.js` (ours: the seeded connection, the worker, the engine, the download card); `vendor/hollama/VENDORED.md` lists every change.
