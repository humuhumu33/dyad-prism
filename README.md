...
`https://humuhumu33.github.io/dyad-prism/ui/` is Hollama (fmaclen/hollama at tag 0.35.4, MIT), built as static files under this subpath and branded with the kit: the warm ground, the accent once per view, Geist, the H. It talks to the OpenAI compatible endpoint this origin's worker answers, so the model runs in the visitor's own browser and no server is anywhere. One link and nothing to set up: the visitor lands in a new session, the connection to this origin is already there, the model starts loading at once with the first download shown, and the picker is filled in. The model it offers is `webgpu:Hologram`, the ladder: the small model answers while the large one loads behind it, and the swap happens when the verified `promote` rule says the large one is resident and measured fast enough. Every answer names the model that produced it, never the rung that was asked for. `scripts/build-hollama.sh` reproduces `shell/ui/` from the pinned commit, `scripts/hollama-brand.patch` (names, links, asset paths, the subpath, the static adapter, the landing), `tools/kit-to-hsl.mjs` (the kit's colours, fonts and mark) and `scripts/ui-host.js` (ours: the seeded connection, the worker, the engine, the download card); `vendor/hollama/VENDORED.md` lists every change.

## The host answers Dyad's own contracts

Dyad's renderer declares every channel with a zod schema and validates nothing it receives, so an answer of the wrong shape reaches a screen and crashes it far from the cause. The build publishes those schemas on the page (`vendor/dyad/hologram.contracts.ts`) and `shell/host.js` parses every answer against the channel's contract: a disagreement names the channel in the console, and a screen that will iterate an array is handed an empty one rather than a value it cannot walk. `tools/contract_shapes.mjs --check` refuses a build whose host answers a channel with a literal of the wrong shape, and runs in the Pages build where the renderer's dependencies are installed.
